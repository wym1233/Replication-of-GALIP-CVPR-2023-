...
class NetG1(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP):
        ...
        self.feature_extract=resnet50(pretrained=False)
        for param in self.feature_extract.parameters():
            param.requires_grad = False
        
        self.fc=nn.Linear(1000,100)
        self.feanorm=nn.LayerNorm(100)
        self.feanorm1=nn.LayerNorm(100)

    def forward(self, realimg,noise, c, eval=False): # x=noise, c=ent_emb
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:

            realimg_fea=self.feature_extract(realimg)
            realimg_fea=self.feanorm(self.fc(realimg_fea))
            realimg_fea=self.feanorm1(realimg_fea+noise)
            
            cond = torch.cat((realimg_fea, c), dim=1)
            out = self.mapping(self.fc_code(realimg_fea).view(realimg_fea.size(0), self.code_ch, self.code_sz, self.code_sz), cond)
            # fuse text and visual features
            for GBlock in self.GBlocks:
                out = GBlock(out, cond)
            out=self.to_rgb(out)
